...
#### Ingen standardverdi for `gap`

`Flex` la tidligere på `gap="m"` uten at du ba om det. Nå er det ingen avstand med mindre du setter `gap` eksplisitt. **Dette endrer utseendet på eksisterende kode uten at du får noen feilmelding.**

```tsx
// Før — ga avstand automatisk
<Flex>

// Etter — sett avstanden eksplisitt
<Flex gap="m">
```
