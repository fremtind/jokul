...
describe("formatKontonummer", () => {
    it("correctly formats 11 digit numbers", () => {
        expect(formatKontonummer("16024454979")).toEqual(
            `1602${nbsp}44${nbsp}54979`,
        );
    });
...
describe("formatKontonummer with partial option", () => {
    it("correctly formats numbers over 4 digits", () => {
        expect(formatKontonummer("16024", { partial: true })).toEqual(
            `1602${nbsp}4`,
        );
        expect(formatKontonummer("1602445", { partial: true })).toEqual(
            `1602${nbsp}44${nbsp}5`,
        );
    });

    it("ignores spaces in otherwise valid inputs", () => {
        expect(formatKontonummer("16 0244 54 979", { partial: true })).toEqual(
            `1602${nbsp}44${nbsp}54979`,
        );
        expect(formatKontonummer("16 0244", { partial: true })).toEqual(
            `1602${nbsp}44`,
        );
    });
});
