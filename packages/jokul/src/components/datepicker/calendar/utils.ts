...
export function getYearOptions(
    ...
    let startYear = currentYear - DEFAULT_YEARS_RANGE;
    let endYear = currentYear + DEFAULT_YEARS_RANGE;

    if (minYear !== undefined) startYear = Math.max(startYear, minYear);
    if (maxYear !== undefined) endYear = Math.min(endYear, maxYear);

    if (startYear > endYear) {
        if (minYear !== undefined && currentYear < minYear) {
            startYear = minYear;
            endYear = minYear;
        } else if (maxYear !== undefined && currentYear > maxYear) {
            startYear = maxYear;
            endYear = maxYear;
        } else {
            startYear = endYear;
