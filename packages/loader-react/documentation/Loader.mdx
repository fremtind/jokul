...
<Ingress>
    ...
    prosesser som tar lengre tid enn ett sekund. For større seksjoner anbefaler vi <em>skeleton loaders</em>, heller enn
    én stor lasteindikator.
</Ingress>

<ComponentExample title="Loader" component={LoaderExample} knobs={loaderExampleKnobs} codeExample={loaderExampleCode} />
...
Du må wrappe innholdet i en [live region](https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA/ARIA_Live_Regions) for at endringer skal plukkes opp av skjermlesere. Wrapperen må også merkes med [`aria-busy="true"`](https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA/Attributes/aria-busy), og sette denne til `"false"` når du erstatter skeleton loaders med det faktiske innholdet.

### Juster hastigheten og fargen på animasjonen
