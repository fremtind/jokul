...
describe("formatFodselsnummer", () => {
    it("correctly formats 11 digit numbers", () => {
        expect(formatFodselsnummer("01065100203")).toEqual(
            `010651${nbsp}00203`,
        );
    });

    it("ignores spaces in otherwise valid inputs", () => {
        expect(formatFodselsnummer("010 65 100 203")).toEqual(
            `010651${nbsp}00203`,
        );
    });
...
describe("formatFodselsnummer with partial option", () => {
    it("correctly formats numbers over 6 digits", () => {
        expect(formatFodselsnummer("0106510", { partial: true })).toEqual(
            `010651${nbsp}0`,
        );
        expect(formatFodselsnummer("010651002", { partial: true })).toEqual(
            `010651${nbsp}002`,
        );
    });
    ...
    it("ignores spaces in otherwise valid inputs", () => {
        expect(
            formatFodselsnummer("010 65 100 203", { partial: true }),
        ).toEqual(`010651${nbsp}00203`);
        expect(formatFodselsnummer("010 65 100", { partial: true })).toEqual(
            `010651${nbsp}00`,
        );
    });
