...
import ExpandExamples, { expandExamplesProps, expandExamplesCode } from "./ExpandExamples";

<Ingress>
    Med en ExpandButton kan vi presentere mye innhold på listen plass. Knappen kan brukes frittstående, og styre
...
Som nevnt i eksempelet for frittstående bruk er det viktig å hjelpe skjermlesere med å forstå at det er en knapp som kommer til å påvirke et stykke innhold. Inspisér gjerne HTMLen som genereres av eksempelet og se hvilke `aria`-attributter som settes, og hvilken verdi de har i åpen og lukket tilstand.

På selve knappen ser du sannsynligvis:

-   `aria-expanded="false"` når lukket og `aria-expanded="true"` når åpen
