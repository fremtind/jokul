...
import { ContentToggleExample, contentToggleExampleCode } from "./ContentToggleExample";
...
-   Bytte fra ett ikon til et annet, som ikke er dekket av [Icons](/komponenter/icons) (men [del gjerne at det er et behov](/komigang/prosessen)!)
...
ContentToggle komponenten gjør sitt beste på å gjette hvor store elementene inni er, så den animerer riktig avstand. Det er basert på fontstørrelsen fra foreldrekomponenten. Så om du animerer `jkl-lead`, sørg for at den klassen er satt over ContentToggle komponenten.

I noen tilfellet så treffer den ikke helt. For å fortelle ContentToggle at den skal bevege seg mer eller mindre, så kan du sette `font-size` på ContentToggle elementet via `className`.
