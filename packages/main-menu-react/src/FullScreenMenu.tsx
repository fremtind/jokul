...
import { IsActiveFunction } from "./contexts/mainMenuContext";
...
export const FullScreenMenu: React.FC<FullScreenMenuProps> = ({
    ...
    const isActive = isOpen || (isActiveFunction && isActiveFunction(baseItem.basePath || "", menuIsOpen));

    const buttonClassName = classNames(className, {
        [`${activeClassName}`]: isActive,
    });
    const menuClassName = classNames({
        "jkl-full-screen-menu": true,
        "jkl-full-screen-menu--open": isOpen,
    });
    ...
    return (
        <>
            ...
            <CoreToggle
                ...
                hidden
            >
                <div className="jkl-full-screen-menu__wrapper">
                    {previousItem && (
                        <button
                            data-text={previousItem.linkText}
                            aria-label={`Tilbake til ${previousItem.linkText}`}
                            className="jkl-full-screen-menu__back-button"
                            onClick={onNavigateBackward}
                        >
                            ←
                        </button>
                    )}
                    <ul className="jkl-full-screen-menu__items">
