...
const getPackageStats = async (packageName: string): Promise<{ package: string; downloads: number }> =>
    new Promise((resolve, reject) => {
        stats.get.lastMonth(
            `@fremtind/${packageName}`,
            (err: unknown, res: Partial<{ package: string; downloads: number }>) => {
                if (err) {
                    return reject(err);
                }
                if (!res?.package) {
        ...
        async () => {
            try {
                ...
                for (const component of components.docs) {
                    const { reactPackageName: packageName, title, id } = component;

                    if (!packageName) {
                        continue;
                    }

                    const componentStats = allStats.docs.find((d) => d.package === `@fremtind/${packageName}`);
                    ...
                    if (!lastUpdated) {
                        ...
                        const ageInHours = (Date.now() - parseISO(lastUpdated).getTime()) / (60 * 60 * 1000);
                        ...
                        if (ageInHours >= 23) {
                            ...
                            console.log(`Updated stats for package ${packageName}`);
                        }
                    }
                }
            } catch (e) {
