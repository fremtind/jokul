...
function isExampleStep(child: {} | null | undefined): child is React.ReactElement<ExampleStepProps> {
    return React.isValidElement<ExampleStepProps>(child) && Boolean(child.props.children);
}

export const SteppedExample: FC<WithChildren> = ({ children }) => {
    const [currentStep, setCurrentStep] = useState(0);
    const previousStep = usePreviousValue(currentStep);
    const steps = React.Children.map(children, (child) => isExampleStep(child) && child.props)?.filter(Boolean);

    if (!steps) {
        return null;
    }

    const goToPreviousStep = () => setCurrentStep((prevValue) => Math.max(prevValue - 1, 0));
    const goToNextStep = () => setCurrentStep((prevValue) => Math.min(prevValue + 1, steps.length - 1));

    const currentExample = steps[currentStep];
    const isFullWidth = !currentExample.headline && !currentExample.description;

    return (
        <div className="jkl-portal-stepped-example" aria-live={previousStep !== currentStep ? "polite" : undefined}>
            {currentExample.headline && (
                <p className="jkl-portal-stepped-example__headline">{currentExample.headline}</p>
            )}
            <div
                className={classNames("jkl-portal-stepped-example__example", {
                    "jkl-portal-stepped-example__example--full-width": isFullWidth,
                })}
            >
