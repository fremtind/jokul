...
import { type Component, type Media, type MainMenu, type MenuItem, type SubMenu } from "payload/generated-types";
import React, { type FC } from "react";
import { useMemo } from "react";
import { type PageSectionProps, assertSectionIs } from "~/block-renderers/page-sections/types";
import { RichText } from "~/components/rich-text";
import { camelShyte, useMainMenu } from "~/utils";
import { useTheme } from "~/utils/useTheme";

function findComponentSlugs(fields: MainMenu["items"], root?: string): { [key: string]: string }[] {
    return (fields || [])
        .map((field) => {
...
export const ComponentOverviewSection: FC<PageSectionProps> = ({ pageSection }) => {
    ...
    const slugMap = useMemo(() => slugs.reduce((map, { id, slug }) => ({ ...map, [id]: slug }), {}), [slugs]);
        ...
        <div className={"jkl-portal-component-overview"}>
            ...
            <div className={"jkl-portal-component-overview-list"}>
                {pageSection.components?.map(({ component: _component }) => {
                    ...
                    const image =
                        prefersColorScheme === "dark" ? component.meta?.imageDark : component.meta?.imageLight;
