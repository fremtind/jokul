...
export const DoDontContentBlock: FC<ContentBlockProps> = ({ contentBlock }) => {
    ...
    if (!contentBlock["doDontItem"] || contentBlock["doDontItem"].length === 0) {
        return null;
    }
    ...
    return (
        <DoDontPair className="jkl-spacing-24-32--bottom">
            {contentBlock["doDontItem"].map((item) => (
                <DoDontExample key={item.id} description={item.description} type={item.type}>
                    {item.variant === "code" && item.code && (
                        <LiveProvider code={item.code} noInline={item.noinline ?? false}>
                            <LivePreview />
                        </LiveProvider>
                    )}
                    {item.variant === "image" && item.image && <Image src={item.image} alt="" />}
                </DoDontExample>
            ))}
        </DoDontPair>
    );
