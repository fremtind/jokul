import{j as e}from"./iframe-BaF8H7Ap.js";import{useMDXComponents as s}from"./index-Dqyz-zOt.js";import{a as i,b as o}from"./blocks-CQpXYXTL.js";import"./preload-helper-PPVm8Dsz.js";import"./index-CuLST3KG.js";import"./index-Cc7Anxxf.js";import"./index-CBNQb1JN.js";const l=`# @fremtind/jokul
...
Noen Jøkul-komponenter (foreløpig \`Button\`, som proof of concept) kan sende
brukshendelser til ditt eget Mixpanel-prosjekt. Dette gir teamet ditt gratis
produktanalyse (klikk osv.) uten å måtte skrive egen sporingskode, samtidig
som Jøkul-teamet kan få innsikt i hvordan komponentene faktisk brukes.

Funksjonaliteten er trygg som standard: uten en \`JokulTrackingProvider\` skjer
det ingenting, og selv med provideren kreves eksplisitt samtykke
(\`consentGranted\`) før noe sendes.

\`\`\`jsx
import { JokulTrackingProvider } from "@fremtind/jokul/tracking";
...
function App() {
    ...
    return (
        <JokulTrackingProvider
            mixpanelToken={process.env.MIXPANEL_TOKEN}
            appName="mitt-produkt"
            consentGranted={consents.statistics === "accepted"}
        >
            {/* Resten av appen din */}
        </JokulTrackingProvider>
    );
...
Merk: \`mixpanel-browser\` er en valgfri peer-avhengighet. Du må installere den
selv (\`npm install mixpanel-browser\`) for å kunne bruke \`JokulTrackingProvider\`
— den lastes kun dynamisk når sporing faktisk aktiveres, og påvirker ikke
bunt-størrelsen for team som ikke bruker funksjonaliteten.

\`Button\` sender automatisk en klikk-hendelse (\`variant\`, om knappen har ikon,
og om den er deaktivert) når den brukes inni en \`JokulTrackingProvider\` med
gyldig samtykke og token. Du kan legge til en PII-fri, menneskelesbar
beskrivelse med \`trackingLabel\`, eller skru av sporing for en enkelt knapp med
\`trackingDisabled\`.

\`\`\`jsx
<Button trackingLabel="lagre-soknad" onClick={handleSubmit}>
    Send inn søknad
</Button>
\`\`\`

### Jøkul-teamets egen aggregerte bruksstatistikk

I tillegg til din egen sporing (over) sender \`JokulTrackingProvider\` som
**standard** en minimal, anonymisert versjon av de samme hendelsene til
Jøkul-teamets eget, sentrale Mixpanel-prosjekt — helt uavhengig av om du
setter opp din egen \`mixpanelToken\` eller ikke. Dette gir oss i Jøkul-teamet
aggregert innsikt i hvordan komponentene faktisk brukes på tvers av alle
forbrukende apper, slik at vi kan prioritere riktig.

Det som sendes til Jøkuls prosjekt er strengt begrenset til et allow-listet
sett med felter per hendelse (f.eks. \`component\`, \`variant\`, \`hasIcon\`,
\`disabled\` for \`Button\`-klikk). **\`appName\` og \`trackingLabel\` sendes aldri**
til Jøkuls prosjekt — kun til ditt eget, hvis du har konfigurert det.

Denne delen av sporingen styres av samme \`consentGranted\`-flagg som din egen
sporing (ingen egen samtykke-mekanisme), og er dermed like trygg som
standard: uten samtykke skjer det ingenting.

Ønsker du ikke å dele aggregert bruksdata med Jøkul-teamet, kan du skru det
av eksplisitt:

\`\`\`jsx
<JokulTrackingProvider
    consentGranted={consents.statistics === "accepted"}
    disableJokulTelemetry
>
    {/* Resten av appen din */}
</JokulTrackingProvider>
\`\`\`

For en full liste over hendelser og typer, se [tracking-mappa](src/tracking/).
`;function r(n){return e.jsxs(e.Fragment,{children:[e.jsx(i,{title:"Om Jøkul"}),`
`,e.jsx(o,{children:l})]})}function f(n={}){const{wrapper:t}={...s(),...n.components};return t?e.jsx(t,{...n,children:e.jsx(r,{...n})}):r()}export{f as default};
