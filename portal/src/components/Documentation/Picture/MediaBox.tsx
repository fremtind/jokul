...
const MediaBox: React.FC<Props> = ({
    ...
    return (
        <div
            ref={ref}
            className={`jkl-portal-media-box ${
                rightAlign ? "jkl-portal-media-box--right" : ""
            }`}
        >
            <motion.figure
                ref={mediaRef}
                className={`jkl-portal-media-box__media ${
                    isInViewport ? "jkl-portal-media-box__media--in-view" : ""
                }`}
            >
                {media}
                {caption && (
                    <figcaption className="jkl-micro jkl-portal-media-box__caption">
                        {caption}
                    </figcaption>
                )}
            </motion.figure>
