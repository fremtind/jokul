...
export function StoryInput(props: ObjectInputProps) {
    ...
    >({});
    const [storyId, setStoryId] = useState<string>(value?.id || "");
    const [storyName, setStoryName] = useState<string>(value?.title || "");
    ...
    const [storyInert, setStoryInert] = useState<boolean>(value?.inert || true);
        ...
        <Stack space={5}>
            ...
            <Card shadow={1} padding={3}>
                <Inline space={2}>
                    <Switch
                        checked={storyInert}
                        onChange={() => setStoryInert(!storyInert)}
                    />
                    <Label htmlFor={`storybook-story-${fieldId}-interaction`}>
            ...
            <Stack space={2}>
                ...
                <iframe
                    ...
                    style={{
                        width: "100%",
                        height: storyHeight,
                        border: "1px solid color-mix(in srgb, transparent 85%, currentColor)",
                    }}
                />
            </Stack>
        </Stack>
