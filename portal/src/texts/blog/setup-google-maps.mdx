...
images:
    - images/google.maps.MapTypeId.ROADMAP.png
    - images/google.maps.MapTypeId.SATELLITE.png
---

<Ingress>
    Let's create a comprehensive project and deliver all the secrets of Google Maps + TypeScript + React? This is the
    first blog of Google Maps tutorial series with a course video.
</Ingress>
...
```typescript
return (
    <div className="App">{scriptLoaded && <Map mapType={google.maps.MapTypeId.ROADMAP} mapTypeControl={true} />}</div>
);
```
...
<PortalImage alt="Roadmap Example" src={props.images[0]} />

<PortalImage alt="Satellite Example" src={props.images[1]} />
