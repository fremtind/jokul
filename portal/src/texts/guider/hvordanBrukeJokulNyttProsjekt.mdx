...
-   [@forbrukerradet/jkl-core](https://github.com/fremtind/jokul/packages/994094)
-   [@forbrukerradet/jkl-webfonts](https://github.com/fremtind/jokul/packages/994096)
-   [@forbrukerradet/browserslist-config-jkl](https://github.com/fremtind/jokul/packages/994092)
...
```sh
yarn add @forbrukerradet/jkl-core @forbrukerradet/jkl-webfonts
yarn add --dev @forbrukerradet/browserslist-config-jkl
```
...
Ta [Accordion](/komponenter/accordion) som et eksempel. Skal du bruke React-komponenten installerer du den med `yarn add @forbrukerradet/jkl-accordion-react` og får automatisk med riktig versjon av `@forbrukerradet/jkl-accordion` som en avhengighet. Dersom du bare bruker stilpakken, installerer du den med `yarn add @forbrukerradet/jkl-accordion`.
...
import { Accordion, AccordionItem } from "@forbrukerradet/jkl-accordion";
```

## Initialisering

For at komponentene i Jøkul skal virke riktig trenger de å vite om brukeren navigerer i løsningen med mus, tastatur eller touch. Det gjøres ved å kjøre `initTabListener` funksjonen fra `@forbrukerradet/jkl-core`. Det trengs bare gjøres én gang, helst så tidlig som det lar seg gjøre. Normalt vil det være i entrypointet i applikasjonen din.
...
@use "~@forbrukerradet/jkl-webfonts/webfonts.scss" with (
