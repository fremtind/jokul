...
For å vise eksempler på riktig og feil bruk kan du bruke hjelpekomponenten `DoDontExample`. Denne kan du bruke i komponentdokumentasjonen uten å måtte importere den eksplisitt. Komponenten tar inn type eksempel (`do` eller `dont`), eksempelet, og en tekst som beskriver bruken.
